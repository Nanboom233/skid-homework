...
fn pack_i420_preview(
    ...
    let expected_payload_len =
        frame_protocol::compute_i420_payload_len(preview_width, preview_height);
    let mut payload = Vec::with_capacity(expected_payload_len);

    if factor == 1 {
        frame_protocol::append_plane_contiguous(
            &mut payload,
            y_plane,
            preview_width,
            preview_height,
    ...
    frame_protocol::append_downsampled_plane_by_factor(
        &mut payload,
        y_plane,
        preview_width,
        preview_height,
