...
fn row_luminance_variance(img: &RgbaImage) -> f64 {
    ...
    let var = row_means.iter().map(|v| (v - mean) * (v - mean)).sum::<f64>() / row_means.len() as f64;
...
fn vertical_edge_angle(img: &RgbaImage) -> f64 {
    ...
    let luma = |x: i32, y: i32| -> f64 {
        let p = img.get_pixel(x.clamp(0, w - 1) as u32, y.clamp(0, h - 1) as u32).0;
...
fn run() -> Result<(), String> {
    ...
    let img = if rotation != 0 {
        ...
        eprintln!("Rotated {}° → {}x{}", rotation, rotated.width(), rotated.height());
        rotated
    } else {
        raw_img
    };
    ...
    for mode in &modes {
        ...
        let result =
            run_native_postprocess_model_with_runtime_hints(resource_hint.clone(), None, &img, None, mode)
                .map_err(|e| format!("{} mode failed: {e}", mode))?;

        let filename = format!("output-{}.png", mode);
        let out_path = output_dir.join(&filename);
        result.image.save(&out_path).map_err(|e| format!("save failed: {e}"))?;
    ...
    eprintln!("╠══════════════════════════════════════════════════════════════════════════════════════════╣");
    eprintln!("║ Input: {} ({}x{} → {}x{} after rotation)                                    ",
    ...
    for r in &results {
        let (mae, rmse, peak) = if r.name == "none" {
            ...
            (compute_mae(baseline, &r.image), compute_rmse(baseline, &r.image), compute_peak_diff(baseline, &r.image))
        };
    ...
    if results.len() >= 2 {
        ...
        if none_img.width() == eq_img.width() && none_img.height() == eq_img.height() {
            ...
            diff_map.save(&diff_path).map_err(|e| format!("save diff failed: {e}"))?;
