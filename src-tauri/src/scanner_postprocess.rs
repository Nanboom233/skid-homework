...
pub async fn tauri_scanner_refine_document_corners(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        if request.source_bytes.len() > MAX_IMAGE_BYTES {
            return Err(format!(
                "Refine corners: input size {} bytes exceeds limit of {} bytes.",
                request.source_bytes.len(), MAX_IMAGE_BYTES,
            ));
        }
        ...
        if let Ok(reader) = ImageReader::new(std::io::Cursor::new(&request.source_bytes)).with_guessed_format() {
            if let Ok((w, h)) = reader.into_dimensions() {
                ...
                if pixel_count > MAX_PIXELS {
                    return Err(format!(
                        "Refine corners: header dimensions {w}x{h} ({pixel_count} pixels) exceed limit of {MAX_PIXELS}.",
                    ));
                }
            }
        }
...
pub fn postprocess_image_bytes_with_options(
    ...
    process_image_request(request, resource_dir_hint, None, app_config_dir_hint)
}
    ...
    if let Ok(reader) = ImageReader::new(std::io::Cursor::new(bytes)).with_guessed_format() {
        if let Ok((w, h)) = reader.into_dimensions() {
            ...
            if header_pixels > MAX_PIXELS {
                return Err(format!(
                    "{}: header dimensions {}x{} ({} pixels) exceed limit of {}.",
                    context, w, h, header_pixels, MAX_PIXELS,
                ));
            }
        }
    }
...
fn process_image_request(
    request: ScannerPostProcessRequest,
    resource_dir_hint: Option<std::path::PathBuf>,
    installed_assets_current_dir_hint: Option<std::path::PathBuf>,
    app_config_dir_hint: Option<std::path::PathBuf>,
) -> Result<(ScannerPostProcessResponse, Vec<u8>), String> {
    ...
    let effective_document_points = normalize_document_points(request.document_points.as_deref())?;
    ...
    if let Some(ref points) = effective_document_points {
        ...
        if request.perspective_transform {
            ...
            let (target_w, target_h, projection, _from, _to) = compute_document_projection(
                points,
            ...
            current = warped;
            log::debug!(
                "[Pipeline] step1:perspective -> {}x{} ({:.1}ms)",
                current.width(),
                current.height(),
                perspective_ms.unwrap()
            );
            if let Some(ref dir) = debug_dir {
            ...
            if max_x > min_x && max_y > min_y {
                let cropped =
                    image::imageops::crop_imm(&current, min_x, min_y, max_x - min_x, max_y - min_y)
                        .to_image();
                current = cropped;
                log::debug!(
                    "[Pipeline] step1:crop -> {}x{}",
                    current.width(),
                    current.height()
                );
            }
    ...
    if request.spine_flattening {
        if postprocess_backend == ScannerPostProcessBackend::NativeMlV1 {
            let result = attempt_residual_control_point_stage(
                ...
                installed_assets_current_dir_hint.clone(),
            ...
            if let Some(image) = result.image {
                ...
                log::debug!(
                    ...
                    current.width(),
                    current.height(),
                    model_ms.unwrap_or(0.0),
                    residual_warp_ms.unwrap_or(0.0),
                );
                if let Some(ref dir) = debug_dir {
    ...
    if request.image_enhancement {
        ...
        log::debug!(
            "[Pipeline] step3:enhance mode={} ({:.1}ms)",
            request.color_mode,
            enhance_ms.unwrap()
        );
    }
...
fn compute_document_projection(
    ...
    let (adj_tl, adj_tr, adj_br, adj_bl) = if h_diff_pct > 5.0 {
        ...
        log::debug!(
            "[Perspective] edge diff={:.1}% < 5% -> no adjustment",
            h_diff_pct
        );
        (*tl, *tr, *br, *bl)
    };
...
///
/// rotate90:  src(x,y) -> dst(H-1-y, x)   output dims: HxW
/// rotate180: src(x,y) -> dst(W-1-x, H-1-y) output dims: WxH
/// rotate270: src(x,y) -> dst(y, W-1-x)   output dims: HxW
...
///
/// ```text
///   0deg:   [TL, TR, BR, BL]  -> identity
///  90deg CW: source BL->TL, TL->TR, TR->BR, BR->BL  -> [3, 0, 1, 2]
/// 180deg:   source BR->TL, BL->TR, TL->BR, TR->BL  -> [2, 3, 0, 1]
/// 270deg CW: source TR->TL, BR->TR, BL->BR, TL->BL  -> [1, 2, 3, 0]
/// ```
fn reorder_points_after_rotation(points: [ScannerPoint; 4], rotation: u16) -> [ScannerPoint; 4] {
    match rotation {
...
mod tests {
    ...
    fn processes_encoded_source_bytes_end_to_end() {
        ...
        assert!(encoded.starts_with(&[0x89, b'P', b'N', b'G']));
        assert_eq!(response.input_width, 24); // rotated: 40x24 @90deg -> 24x40
    ...
    fn warp_document_to_rect_no_margin() {
        ...
        let (target_w, target_h, _projection, _from, _to) =
            compute_document_projection(&points, 240.0, 180.0).expect("projection should succeed");
        let (max_width, max_height) = compute_true_aspect_ratio_dimensions(&points, 240.0, 180.0);
