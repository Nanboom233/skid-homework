...
pub fn detect_document_opencv(
    ...
    let dynamic_image = match crate::scanner_detect::build_dynamic_image_from_preview_frame_packet(
        &packet,
    ) {
        Ok(img) => img,
        Err(e) => {
...
pub fn detect_contour_quad(
    ...
    best_points.map(|pts| {
        pts.iter()
            .map(|p| ScannerPoint { x: p.x as f32, y: p.y as f32 })
            .collect()
    })
}
...
fn compute_quad_score(
    ...
    let border_touch_penalty =
        compute_border_touch_penalty(points, frame_width, frame_height);
...
fn compute_border_touch_penalty(
    points: &[Point; 4],
    frame_width: f64,
    frame_height: f64,
) -> f64 {
