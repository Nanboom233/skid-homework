...
pub fn describe_native_postprocess_model_with_hints(
    ...
    describe_native_postprocess_model_with_runtime_hints(resource_dir_hint, None, None)
}
...
pub fn describe_native_postprocess_model_with_runtime_hints(
    ...
    let Some(selected_resource_root) = scanner_resource::select_resource_root(
        &resource_root_candidates,
        &POSTPROCESS_INTERESTING_PATHS,
    ) else {
    ...
    let session_snapshot = ensure_postprocess_session(
        ...
        &model,
        &shared_ort_context.available_providers,
    );
...
fn ensure_postprocess_session(
    ...
    let same_model_loaded = state
        ...
        .unwrap_or(false);
    let same_providers_loaded = state.session_providers == available_providers;

    if state.session.is_some() && same_model_loaded && same_providers_loaded {
        return PostprocessOrtSessionSnapshot {
            ready: true,
            session_error: None,
    ...
    match session_result {
        Ok(session) => {
            ...
            state.session_providers = available_providers.to_vec();
...
pub fn run_native_postprocess_model_with_runtime_hints(
    ...
    #[cfg(debug_assertions)]
    {
        ...
        if plane > 0 && grid.len() >= plane * 2 {
            ...
            for gx in [0, gw / 4, gw / 2, 3 * gw / 4, gw.saturating_sub(1)] {
                if gx < gw {
                    ...
                    let id_x = if gw <= 1 {
                        ...
                        2.0 * gx as f32 / (gw as f32 - 1.0) - 1.0
                    };
            ...
            log::debug!(
                "[UVDoc grid diag] ch0 along mid-row(gy={}): {}",
                mid_gy,
                row_samples.join(", ")
            );
            ...
            for gy in [0, gh / 4, gh / 2, 3 * gh / 4, gh.saturating_sub(1)] {
                if gy < gh {
                    ...
                    let id_y = if gh <= 1 {
                        ...
                        2.0 * gy as f32 / (gh as f32 - 1.0) - 1.0
                    };
            ...
            log::debug!(
                "[UVDoc grid diag] ch1 along mid-col(gx={}): {}",
                mid_gx,
                col_samples.join(", ")
            );
    ...
    let image = apply_uvdoc_point_grid(
        proxy_image,
        target_size,
        grid_postprocess,
        &point_grid.1,
...
fn apply_uvdoc_point_grid(
    ...
    let out_w = target_size
        .map(|s| s.0)
        .unwrap_or_else(|| proxy_image.width());
    let out_h = target_size
        .map(|s| s.1)
        .unwrap_or_else(|| proxy_image.height());
