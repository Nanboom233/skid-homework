...
pub fn resolve_camera_server_artifact_from_candidates(
    installed_jar_hint: Option<PathBuf>,
    resource_dir_hint: Option<PathBuf>,
) -> Result<CameraServerArtifact, String> {
    if let Some(installed_jar) = installed_jar_hint {
        if installed_jar.is_file() {
            let install_root = installed_jar.parent().ok_or_else(|| {
                format!(
                    "Installed camera-server.jar has no parent directory: {}.",
                    path_to_string(&installed_jar)
                )
            })?;
            let canonical = validate_path_containment(&installed_jar, install_root)?;
            return Ok(CameraServerArtifact {
                path: path_to_string(&canonical),
                source: "scanner-camera-asset".to_string(),
            });
        }
    }

    resolve_camera_server_artifact_from_resource_dir(resource_dir_hint)
}

pub fn resolve_camera_server_artifact(app: &AppHandle) -> Result<CameraServerArtifact, String> {
    resolve_camera_server_artifact_from_candidates(
        scanner_assets::installed_camera_server_jar_from_app(app),
