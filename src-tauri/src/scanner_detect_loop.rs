...
fn default_stable_frames() -> usize {
    8
}
fn default_variance_threshold() -> f32 {
    8.0
}
fn default_stable_hold_ms() -> u64 {
    1200
}
fn default_miss_grace_frames() -> u32 {
    3
}
fn default_miss_grace_ms() -> f64 {
    360.0
}
fn default_smoothing_threshold_px() -> f32 {
    18.0
}
...
pub async fn tauri_scanner_start_detection_loop(
    ...
    let installed_assets_current_dir = scanner_assets::installed_assets_current_dir_from_app(&app);
...
async fn detection_loop(
    ...
    installed_assets_current_dir: Option<std::path::PathBuf>,
    ...
    loop {
        ...
        let (points, detection_ms, frame_width, frame_height, message) = if backend == "native-ort"
        {
            run_native_ort_detect(
                &resource_dir,
                &installed_assets_current_dir,
                &app_config_dir,
            )
        ...
        let quad: Option<[ScannerPoint; 4]> = points.as_ref().and_then(|pts| {
            if pts.len() == 4 {
        ...
        let in_cooldown = auto_capture_cooldown_until.map_or(false, |until| Instant::now() < until);

        let auto_capture_triggered = stable
            && presence_state.effective_detected
            && !in_cooldown
            && stable_since.map_or(false, |since| since.elapsed() >= stable_hold);
...
fn run_native_ort_detect(
    ...
    match detect_document_native_ort(
        request,
        resource_dir.clone(),
        installed_assets_current_dir.clone(),
