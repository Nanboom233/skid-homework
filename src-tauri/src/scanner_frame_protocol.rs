...
pub fn parse_frame_packet_header(packet: &[u8]) -> Result<(u32, u32, &[u8]), String> {
    ...
    if codec != FRAME_CODEC_I420 && codec != FRAME_CODEC_I420_TELEMETRY {
        return Err(format!("Expected an I420 frame packet, got codec {codec}."));
    }
...
pub fn decode_i420_payload_to_rgb_image(
    ...
    let chroma_len = chroma_width.checked_mul(chroma_height).ok_or_else(|| {
        format!(
            "I420 chroma dimension overflow: {}x{}",
...
pub fn build_dynamic_image_from_preview_frame_packet(
    packet: &[u8],
