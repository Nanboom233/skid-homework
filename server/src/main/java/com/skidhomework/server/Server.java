...
import com.skidhomework.server.camera.Camera1Session;
import com.skidhomework.server.camera.Camera2Session;
import com.skidhomework.server.camera.ICameraSession;
import com.skidhomework.server.encoder.IEncoder;
import com.skidhomework.server.encoder.Nv21Encoder;
import com.skidhomework.server.encoder.SurfaceEncoder;
...
public final class Server {
    ...
    public static void main(String[] args) {
        ...
        AtomicReference<ICameraSession> activeCaptureRef = new AtomicReference<>();
        SocketRelay relay = null;
        StillCapture.StatusServer stillCaptureServer = null;
        StillCapture.StreamServer stillStreamServer = null;
        ...
        Consumer<StopReason> requestTerminalStop = (reason) -> {
            ...
            boolean processShutdown = "process_shutdown".equals(safeReason.getCode());
            if (safeReason.isSocketTerminal() || processShutdown) {
        ...
        try {
            ...
            // Keep the preview socket open for the server lifetime so clients can reconnect.
            ...
            stillCaptureServer = new StillCapture.StatusServer(
                    config.stillSocketName,
                    activeCaptureRef
            );
            stillCaptureServer.start();
            stillStreamServer = new StillCapture.StreamServer(
                    config.stillStreamSocketName,
                    activeCaptureRef
            );
            stillStreamServer.start();

            // --- Accept loop: each accepted preview client gets a transport handshake. ---
            while (!shutdownRequested.get()) {
                ...
                // Write a 1-byte transport handshake before camera startup.
                ...
                try {
                    ...
                    System.out.println("[Server] Client disconnected before handshake. Re-accepting...");
            ...
            if (stillStreamServer != null) {
                stillStreamServer.close();
            }
            SocketRelay activeRelay = relayRef.getAndSet(null);
            if (activeRelay != null) {
                activeRelay.close();
    ...
    private static StopReason runStreamingLoop(
            ServerConfig config,
            SocketRelay relay,
            AtomicReference<ICameraSession> activeCaptureRef,
        ...
        while (!shutdownRequested.get()) {
            ...
            try {
                ...
                if (useLegacyPreviewFallback) {
                    System.out.println("[Server] Using Camera1 + Nv21Encoder pipeline.");
                    Camera1Session camera1Session = new Camera1Session(
                            config.cameraId,
                            config.width,
                            config.height,
                            requestSessionStop
                    );
                    capture = camera1Session;
                    capture.start();

                    Nv21Encoder nv21Encoder = new Nv21Encoder(
                            camera1Session.getPreviewWidth(),
                            camera1Session.getPreviewHeight(),
                            config.bitrate,
                            config.framerate,
                            relay,
                            requestSessionStop
                    ...
                    encoder = nv21Encoder;
                    camera1Session.attachEncoder(nv21Encoder);
                } else {
                    System.out.println("[Server] Using Camera2 + SurfaceEncoder pipeline.");
                    System.out.println("[Server] Creating SurfaceEncoder with size: " + config.width + "x" + config.height);
